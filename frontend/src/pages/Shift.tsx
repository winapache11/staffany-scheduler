...
//import MuiWeekPicker from "./MuiWeekPicker.tsx";
...
const Shift = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const { results } = await getShifts(search);
        setRows(results);
      } catch (error) {
  ...
  const onHandleWeekPicker = (startDate: string, endDate: string) => {
    const stringTo = "/shift?startdate=" + startDate + "&enddate=" + endDate;
    history.push(stringTo);
  }
      ...
      <Grid item xs={12}>
        <Card className={classes.root}>
          <CardContent>
            ...
            <div><WeekPicker onHandleWeekPicker={onHandleWeekPicker} /></div>
            <DataTable
              title="Shifts"
              columns={columns}
              data={rows}
